...
END_OF_TEXT;
    ...
    $add_master_sql = "INSERT INTO master_name (date_added, date_modified, f_name, l_name) VALUES
    ...
    if (($_POST['address']) ||($_POST['city']) || ($_POST['state']) ||($_POST['zipcode'])) {
        ...
        $get_order_sql = "SELECT st.id, si.item_title, si.item_price, st.sel_item_qty, st.sel_item_size, st.sel_item_color
                FROM store_shoppertrack AS st 
                LEFT JOIN store_items AS si 
                ON si.id = st.sel_item_id
                WHERE session_id = '".$_COOKIE['PHPSESSID']."'";
                
        $get_order_res = mysqli_query($mysqli, $get_order_sql) or die(mysqli_error($mysqli));
        while ($cart_info = mysqli_fetch_array($get_order_res)) {
            // Prepare sql for update stock
            $update_stock_sql = "UPDATE store_item_stock
                        SET stock_sold_qty = '".$cart_info['sel_item_qty']."'
